...
extern inline void glob_rng_init(void);

inline urint_t random_uint(void);

extern inline bool coin_flip(void);

extern inline urint_t random_uint_upto(const urint_t rbound);

extern inline rint_t random_int_in_range(const rint_t lbound,
					 const rint_t rbound);

extern inline double random_dbl_upto(const double rbound);

extern inline double random_dbl_in_range(const double lbound,
					 const double rbound);

extern inline void init_random_int_array(rint_t *const restrict array,
					 const size_t length);
extern inline rint_t *create_random_int_array(const size_t length);

extern inline void init_random_int_array_in_range(rint_t *const restrict array,
						  const size_t length,
						  const rint_t lbound,
						  const rint_t rbound);
extern inline rint_t *create_random_int_array_in_range(const size_t length,
						       const rint_t lbound,
						       const rint_t rbound);

extern inline void shuffle_array(void *const restrict array,
