...
#define ERROR_EXPECTED_JOIN_CLOSE_HEADER				\
PARSE_ERROR_HEADER("expected " INPUT_WRAP("'+'") ERROR_OPEN ", GROUP "	\
		   "flag, COLUMN flag, TABLE flag, DATABASE flag, or "	\
		   "end of arguments instead of")
...
expected_join_close(const struct GenerateArgvState *const restrict argv)
{
	...
	char *restrict ptr
	= put_string_size(&buffer[0],
			  ERROR_EXPECTED_JOIN_CLOSE_HEADER,
			  sizeof(ERROR_EXPECTED_JOIN_CLOSE_HEADER) - 1lu);
...
{
	expected_join_close(&state->argv);
	generate_parse_error(state);
}
		    ...
		    GenerateParseNode *const handle_grp_spec)
{
	...
	default:
		error_expected_join_close(state);
		return;
	}
...
parse_join_integer_random(struct GenerateParseState *const restrict state)
{
	...
	if (state->argv.arg.from == state->argv.arg.until) {
		join_integer_random_default(state);
		join_spec_state_close(&state->specs.join);
	...
	case 'g':
		if (*rem == '\0')
			parse_join_integer_random_default_group(state);
		else
			error_invalid_rand_spec(state);
		return;
	...
	case 'f':
		if (*rem == '\0')
			parse_join_integer_random_from(state);
		else
			error_invalid_rand_spec(state);
		return;
	...
	case 'u':
		if (*rem == '\0')
			parse_join_integer_random_upto(state);
		else
			error_invalid_rand_spec(state);
		return;
	...
	case 'r':
		if (*rem == '\0')
			parse_join_integer_random_range(state);
		else
			error_invalid_rand_spec(state);
		return;
	...
	case 'c':
		if (*rem == '\0') {
NEXT_COL_SPEC:		join_integer_random_default(state);
			join_spec_state_close(&state->specs.join);
			parse_next_col_spec(state);
		} else {
			error_invalid_rand_spec(state);
	...
	case 't':
		if (*rem == '\0') {
NEXT_TBL_SPEC:		join_integer_random_default(state);
			join_spec_state_close(&state->specs.join);
			parse_table_complete(state);
			parse_next_tbl_spec(state);
		} else {
	...
	case 'd':
		if (*rem == '\0') {
NEXT_DB_SPEC:		join_integer_random_default(state);
			join_spec_state_close(&state->specs.join);
			parse_database_complete(state);
			parse_next_db_spec(state);
			return;
...
{
	parse_join_complete(state,
			    &join_u_integer_default,
			    &parse_join_u_integer_default_group);
}
...
{
	parse_join_complete(state,
			    &join_u_integer_unique,
			    &parse_join_u_integer_unique_group);
}
...
parse_join_u_integer_fixed(struct GenerateParseState *const restrict state)
{
	...
	struct StubBuilder *const restrict fixed_int
	= &state->specs.col->type_q.u_integer.fixed;
...
{
	parse_join_grp_spec(state,
			    &join_u_integer_random_default_group);
}
...
{
	parse_join_complete(state,
			    &join_u_integer_random_default,
			    &parse_join_u_integer_random_default_group);
}
...
{
	parse_join_grp_spec(state,
			    &join_u_integer_random_from_group);
}
...
parse_join_u_integer_random_from(struct GenerateParseState *const restrict state)
{
	...
	parse_join_complete(state,
			    &join_u_integer_random_from,
			    &parse_join_u_integer_random_from_group);
}
...
{
	parse_join_grp_spec(state,
			    &join_u_integer_random_upto_group);
}
...
parse_join_u_integer_random_upto(struct GenerateParseState *const restrict state)
{
	...
	parse_join_complete(state,
			    &join_u_integer_random_upto,
			    &parse_join_u_integer_random_upto_group);
}
...
{
	parse_join_grp_spec(state,
			    &join_u_integer_random_range_group);
}
...
parse_join_u_integer_random_range(struct GenerateParseState *const restrict state)
{
	...
	parse_join_complete(state,
			    &join_u_integer_random_range,
			    &parse_join_u_integer_random_range_group);
}
...
parse_join_u_integer_random(struct GenerateParseState *const restrict state)
{
	...
	if (state->argv.arg.from == state->argv.arg.until) {
		join_u_integer_random_default(state);
		join_spec_state_close(&state->specs.join);
		generate_parse_complete(state); /* done parsing */
		return;
	}
	...
	case 'g':
		if (*rem == '\0')
			parse_join_u_integer_random_default_group(state);
		else
			error_invalid_rand_spec(state);
		return;
	...
	case 'f':
		if (*rem == '\0')
			parse_join_u_integer_random_from(state);
		else
			error_invalid_rand_spec(state);
		return;
	...
	case 'u':
		if (*rem == '\0')
			parse_join_u_integer_random_upto(state);
		else
			error_invalid_rand_spec(state);
		return;
	...
	case 'r':
		if (*rem == '\0')
			parse_join_u_integer_random_range(state);
		else
			error_invalid_rand_spec(state);
		return;
	...
	case 'c':
		if (*rem == '\0') {
NEXT_COL_SPEC:		join_u_integer_random_default(state);
			join_spec_state_close(&state->specs.join);
			parse_next_col_spec(state);
		} else {
			error_invalid_rand_spec(state);
	...
	case 't':
		if (*rem == '\0') {
NEXT_TBL_SPEC:		join_u_integer_random_default(state);
			join_spec_state_close(&state->specs.join);
			parse_table_complete(state);
			parse_next_tbl_spec(state);
		} else {
	...
	case 'd':
		if (*rem == '\0') {
NEXT_DB_SPEC:		join_u_integer_random_default(state);
			join_spec_state_close(&state->specs.join);
			parse_database_complete(state);
			parse_next_db_spec(state);
			return;
...
parse_join_integer_qualifier(struct GenerateParseState *const restrict state)
{
	...
	case 'g':
		if (*rem == '\0')
			parse_join_integer_default_group(state);
		else
			error_invalid_integer_type_q(state);
		return;
	...
	case 'u':
		if (*rem == '\0')
			parse_join_integer_unique(state);
		else
			error_invalid_integer_type_q(state);
		return;
	...
	case 'f':
		if (*rem == '\0')
			parse_join_integer_fixed(state);
		else
			error_invalid_integer_type_q(state);
		return;
	...
	case 'r':
		if (*rem == '\0')
			parse_join_integer_random(state);
		else
			error_invalid_integer_type_q(state);
		return;
	...
	case 'c':
		if (*rem == '\0') {
NEXT_COL_SPEC:		join_integer_default(state);
			join_spec_state_close(&state->specs.join);
			parse_next_col_spec(state);
		} else {
			error_invalid_integer_type_q(state);
	...
	case 't':
		if (*rem == '\0') {
NEXT_TBL_SPEC:		join_integer_default(state);
			join_spec_state_close(&state->specs.join);
			parse_table_complete(state);
			parse_next_tbl_spec(state);
		} else {
	...
	case 'd':
		if (*rem == '\0') {
NEXT_DB_SPEC:		join_integer_default(state);
			join_spec_state_close(&state->specs.join);
			parse_database_complete(state);
			parse_next_db_spec(state);
			return;
...
parse_join_u_integer_qualifier(struct GenerateParseState *const restrict state)
{
	...
	case 'g':
		if (*rem == '\0')
			parse_join_u_integer_default_group(state);
		else
			error_invalid_u_integer_type_q(state);
		return;
	...
	case 'u':
		if (*rem == '\0')
			parse_join_u_integer_unique(state);
		else
			error_invalid_u_integer_type_q(state);
		return;
	...
	case 'f':
		if (*rem == '\0')
			parse_join_u_integer_fixed(state);
		else
			error_invalid_u_integer_type_q(state);
		return;
	...
	case 'r':
		if (*rem == '\0')
			parse_join_u_integer_random(state);
		else
			error_invalid_u_integer_type_q(state);
		return;
	...
	case 'c':
		if (*rem == '\0') {
NEXT_COL_SPEC:		join_u_integer_default(state);
			join_spec_state_close(&state->specs.join);
			parse_next_col_spec(state);
		} else {
			error_invalid_u_integer_type_q(state);
	...
	case 't':
		if (*rem == '\0') {
NEXT_TBL_SPEC:		join_u_integer_default(state);
			join_spec_state_close(&state->specs.join);
			parse_table_complete(state);
			parse_next_tbl_spec(state);
		} else {
	...
	case 'd':
		if (*rem == '\0') {
NEXT_DB_SPEC:		join_u_integer_default(state);
			join_spec_state_close(&state->specs.join);
			parse_database_complete(state);
			parse_next_db_spec(state);
			return;
